...
const count = (target, input) => {
  // Solution code here...
  let flat = input.reduce((acc,idx)=>acc.concat(idx),[]);
  let count=0;
  let newArr=flat.map(value=>{
    if (value==target){
      count+=1;
    }
  })
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(arr => {
    newArr.push(arr.filter(value=> typeof value== 'number' && value% 5 === 0).map(value=> Math.pow(2, value)))
  })
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filtered = data.filter(charractor => charractor.gender === 'male' || charractor.gender === 'female')
  let names = filtered.map(value => {
    return value.name;
  })
