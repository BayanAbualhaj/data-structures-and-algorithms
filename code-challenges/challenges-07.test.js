...
const addOne = (arr) => {
  const array = arr.map( value => value = value + 1);
  return array;
}
...
const addQuestion = (arr) => {
  const array = arr.map(value => value +"?");
  return array;
}
...
const forEachTwoToThe = (arr) => {
  let arrayX = []
  arr.forEach(value => arrayX.push(Math.pow(2, value)));
  return arrayX;
};
...
const mapTwoToThe = (arr) => {
  let arrayX = []
  arr.map(value => arrayX.push(Math.pow(2, value)));
  return arrayX;
};
